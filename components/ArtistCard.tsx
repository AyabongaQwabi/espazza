  ...
  const togglePlay = async (songUrl: string) => {
    if (currentlyPlaying === songUrl) {
      ...
      try {
        ...
        toast({
          title: "Error",
          description: "Unable to play this audio file. Please try again or check the file.",
          variant: "destructive",
        })
      }
    }
  ...
  const handleShare = (platform: string) => {
    ...
    switch (platform) {
      case "facebook":
        ...
        break
      case "whatsapp":
        shareUrl = `https://wa.me/?text=${encodeURIComponent(`Check out ${artist_name} on XHAP: ${url}`)}`
        break
      case "email":
    ...
    <div className="max-w-4xl mx-auto p-8 bg-gradient-to-br from-gray-900 to-black rounded-xl shadow-2xl">
      <div className="grid md:grid-cols-3 gap-8">
        <div className="md:col-span-1">
          ...
          <h2 className="text-3xl font-bold text-white mt-4 mb-2">{artist_name}</h2>
            ...
            <Dialog open={isShareModalOpen} onOpenChange={setIsShareModalOpen}>
              ...
              <DialogContent>
                ...
                <div className="flex justify-around mt-4">
                  ...
                  <Button onClick={() => handleShare("copy")} className="bg-gray-600 hover:bg-gray-700">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div>
            <h3 className="text-xl font-semibold text-white mb-3">Latest Videos</h3>
            <div className="grid grid-cols-2 gap-4">
              {youtube_links?.slice(0, 4).map((video, index) => (
                ...
                >
                  <Image
                    src={`https://img.youtube.com/vi/${getYouTubeVideoId(video)}/0.jpg`}
                    alt="Video thumbnail"
          ...
          <div>
            <h3 className="text-xl font-semibold text-white mb-3">Demo Songs</h3>
                ...
                >
                  <div className="flex items-center space-x-3">
                    <button
                      ...
                      aria-label={currentlyPlaying === song.url ? "Pause" : "Play"}
                    >
                      {currentlyPlaying === song.url ? <FaPause /> : <FaPlay />}
                    </button>
                    <div>
                      <p className="font-medium text-white">{song.title}</p>
                      <p className="text-sm text-gray-400">Released: {song.releaseDate}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
      ...
      <audio
        ...
        onError={(e) => {
          ...
          toast({
            title: "Error",
            description: "There was an error with the audio file. Please try again.",
            variant: "destructive",
          })
        }}
      />
