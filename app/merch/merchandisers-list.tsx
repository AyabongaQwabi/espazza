...
export default function MerchandisersList({ initialData = [] }: { initialData?: any[] }) {
      ...
      try {
        ...
        if (newMerchandisers && newMerchandisers.length > 0) {
          ...
          setMerchandisers((prevMerchandisers) =>
            pageNumber === 1 ? merchandisersWithImages : [...prevMerchandisers, ...merchandisersWithImages],
          )
  ...
  if (merchandisers.length === 0) {
    return (
      <div className="text-center py-20">
        <h3 className="text-2xl font-semibold text-white mb-4">No merchandisers found</h3>
        ...
        {search && (
          <Button asChild variant="outline">
            <Link href="/merchandisers">View all merchandisers</Link>
          </Button>
        )}
      </div>
    )
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {merchandisers.map((merchandiser, index) => (
          <MerchandiserCard key={merchandiser.id} merchandiser={merchandiser} index={index} />
        ))}
      </div>
...
function MerchandiserCard({ merchandiser, index }: { merchandiser: any; index: number }) {
  const featuredImage =
    ...
    >
      <Card className="h-full flex flex-col overflow-hidden bg-zinc-900 border-zinc-800 hover:border-zinc-700 transition-colors">
        ...
        <CardContent className="flex-1 p-6">
          <h2 className="text-xl font-bold text-white mb-2">{merchandiser.name}</h2>
          <p className="text-zinc-400 text-sm line-clamp-3 mb-4">{merchandiser.description}</p>
          <div className="space-y-2 text-sm">
            <div className="flex items-start">
              <Mail className="h-4 w-4 text-zinc-500 mt-0.5 mr-2" />
              <a href={`mailto:${merchandiser.email}`} className="text-zinc-300 hover:text-white">
                {merchandiser.email}
              </a>
            </div>
            <div className="flex items-start">
              <Phone className="h-4 w-4 text-zinc-500 mt-0.5 mr-2" />
              <a href={`tel:${merchandiser.contact_number}`} className="text-zinc-300 hover:text-white">
        ...
        <CardFooter className="px-6 pb-6 pt-0">
          <div className="w-full flex gap-3">
            <Button asChild className="flex-1 bg-red-600 hover:bg-red-700">
              <Link href={`/merchandisers/${merchandiser.id}`}>View Details</Link>
            </Button>
