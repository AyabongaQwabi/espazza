...
export default function CouponsPage() {
  ...
  async function fetchCouponDetails(id: string) {
    try {
      ...
      toast({
        title: 'Error',
        description:
          error.response?.data?.error || 'Failed to fetch coupon details',
        variant: 'destructive',
      });
    }
  }
  ...
  async function updateCoupon() {
    ...
    try {
      ...
      setCoupons(
        coupons.map((coupon) =>
          coupon.id === selectedCoupon.id ? response.data.coupon : coupon
        )
      );
          ...
          <div className='grid gap-4 py-4'>
            ...
            <div className='grid grid-cols-4 items-center gap-4'>
              ...
              <Input
                ...
                type='number'
                value={
                  formData.usage_limit === null ? '' : formData.usage_limit
                }
                onChange={(e) => {
                  const value =
                    e.target.value === ''
              ...
              <div className='flex items-center space-x-2 col-span-3'>
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    setFormData({ ...formData, one_time_per_user: checked })
                  }
                />
                <Label htmlFor='one_time_per_user'>
                  {formData.one_time_per_user ? 'Yes' : 'No'}
                </Label>
              </div>
            </div>
            <div className='grid grid-cols-4 items-center gap-4'>
          ...
          <div className='grid gap-4 py-4'>
            ...
            <div className='grid grid-cols-4 items-center gap-4'>
              ...
              <div className='flex items-center space-x-2 col-span-3'>
                ...
                />
                <Label htmlFor='edit-one_time_per_user'>
                  {formData.one_time_per_user ? 'Yes' : 'No'}
                </Label>
              </div>
            </div>
                  ...
                  <div className='mt-3 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-md'>
                    <div className='flex items-start'>
                      ...
                      <p className='text-xs text-amber-800 dark:text-amber-300'>
                        This coupon has been used {selectedCoupon.usage_count}{' '}
                        times. It will be deactivated instead of deleted.
                      </p>
                    </div>
                  </div>
                )}
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className='sm:max-w-[600px]'>
          <DialogHeader>
            ...
            <DialogDescription>
              Detailed information about this coupon and its usage.
            </DialogDescription>
          </DialogHeader>
          {detailedCoupon ? (
            <Tabs defaultValue='details'>
                ...
                <div className='flex justify-between items-center'>
                  <div>
                    ...
                    <p className='text-muted-foreground text-sm'>
                      {detailedCoupon.description || 'No description'}
                    </p>
                  </div>
                ...
                <div className='grid grid-cols-2 gap-4 mt-4'>
                    ...
                    <p className='flex items-center'>
                      {detailedCoupon.discount_type === 'percentage' ? (
                        ...
                        <>
                          <DollarSign className='h-4 w-4 mr-1 text-green-500' />
                          <span>
                            R{detailedCoupon.discount_amount.toFixed(2)} off
                          </span>
                        </>
                      )}
                    </p>
                  ...
                  <div className='space-y-2'>
                    ...
                    <p className='flex items-center'>
                      ...
                      <span>
                        {format(new Date(detailedCoupon.created_at), 'PPP')}
                      </span>
                    </p>
                  </div>
                  <div className='space-y-2'>
                    ...
                    <p className='flex items-center'>
                      {detailedCoupon.expiry_date ? (
                        <>
                          ...
                          <span>
                            {format(
                              new Date(detailedCoupon.expiry_date),
                              'PPP'
                            )}
                          </span>
                        </>
                  ...
                  <div className='space-y-2'>
                    ...
                    <p className='flex items-center'>
                      ...
                      <span>
                        {detailedCoupon.one_time_per_user ? 'Yes' : 'No'}
                      </span>
                    </p>
                  </div>
                  <div className='space-y-2'>
              ...
              <TabsContent value='usage' className='py-4'>
                {detailedCoupon.coupon_usage &&
                detailedCoupon.coupon_usage.length > 0 ? (
                  <div className='overflow-x-auto'>
                    <Table>
                      <TableHeader>
                        <TableRow>
                      ...
                      <TableBody>
                        {detailedCoupon.coupon_usage.map((usage) => (
                          <TableRow key={usage.id}>
                            <TableCell>
                              {usage.profiles?.username ||
                                usage.profiles?.email ||
                                usage.user_id}
                            </TableCell>
                            <TableCell>
                              {format(new Date(usage.used_at), 'PPP p')}
                            </TableCell>
                            <TableCell>
